...
#	Combinacion 1
	gmt makecpt -Ccyclic -T0/180  -N -H >  $COLOR -G0.5/1
	gmt makecpt -Ccyclic -T180/360   -H >> $COLOR -G0/0.5

#	Combinacion 2
	#gmt makecpt -Ccyclic -T0/90   -N -H >  $COLOR -G0.75/1 -Ww
	#gmt makecpt -Ccyclic -T90/360    -H >> $COLOR -G0/0.75
...
#	gmt grdimage $CUT -C$COLOR -I$SHADOW
	gmt grdimage $CUT -C$COLOR

#	Agregar escala vertical a partir de CPT (-C). Posición (x,y) +wlargo/ancho. Anotaciones (-Ba). Leyenda (+l). 
	gmt colorbar -Dx0/-1.0+w15/0.618ch -C$COLOR -Ba30f15+l"Orientaci\363n pendiente(\232)"
